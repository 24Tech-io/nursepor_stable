...
const SCANNER_PATTERNS = [/acunetix/i, /netsparker/i, /appscan/i, /webinspect/i];
...
export function isTorIP(ip: string): boolean {
  ...
  const torPatterns = [/^185\.220\./, /^77\.247\./];
...
setInterval(
  () => {
    ...
    if (threatScores.size > MAX_THREAT_HISTORY) {
      const sorted = Array.from(threatScores.entries()).sort(
