...
async function main() {
  ...
  allEnrollments.forEach((e, i) => {
    ...
    console.log(
      `   Flags: isRequestable=${e.is_requestable}, isDefaultUnlocked=${e.is_default_unlocked}, isPublic=${e.is_public}`
    );
    console.log('');
  });
  ...
  for (const e of allEnrollments) {
    ...
    console.log(`${e.title}:`);
    console.log(
      `   Course status "${e.course_status}" matches published/active? ${matchesPublished}`
    );
    console.log(
      `   Enrollment status "${e.enrollment_status}" = "active"? ${matchesEnrollmentActive}`
    );
