...
async function main() {
  ...
  `;
    ...
    enrollmentsQuery.forEach(c => {
        console.log(`   - ${c.title} (ID: ${c.course_id}, Status: ${c.status}, Progress: ${c.progress}%)`);
    });
    console.log('');
    ...
    mergedCourseIds.forEach(id => {
        ...
        const source = fromEnrollments ? (fromProgress ? 'BOTH' : 'enrollments only') : 'studentProgress only';
