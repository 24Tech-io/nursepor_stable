...
async function main() {
    ...
    `;
        ...
        for (const r of requests) {
            ...
            const enr = await sql`SELECT id FROM enrollments WHERE user_id = ${r.student_id} AND course_id = ${r.course_id}`;
            console.log(`  Enrollment: ${enr.length > 0 ? 'EXISTS' : 'MISSING'}`);

            const prog = await sql`SELECT id FROM student_progress WHERE student_id = ${r.student_id} AND course_id = ${r.course_id}`;
