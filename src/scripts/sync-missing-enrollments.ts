  ...
  try {
    ...
    for (const request of approvedRequests) {
      ...
      if (existing.length > 0) {
        console.log(
          `✓ User ${request.student_name} (ID: ${request.student_id}) - "${request.course_title}": Already enrolled`
        );
        alreadyExistsCount++;
        continue;
      }
      ...
      console.log(
        `✅ Created enrollment for ${request.student_name} (ID: ${request.student_id}) - "${request.course_title}"`
      );
      syncedCount++;
