    ...
    `;
        ...
        if (enrollmentsRaw.length > 0) {
            enrollmentsRaw.forEach(e => {
                console.log(`   - ID:${e.id}, Course:"${e.title}", EnrollStatus:${e.status}, CourseStatus:${e.course_status}, Progress:${e.progress}%`);
            });
        }
        console.log('');
    ...
    `;
        ...
        if (progressRaw.length > 0) {
            progressRaw.forEach(p => {
                console.log(`   - Course:"${p.title}", CourseStatus:${p.course_status}, Progress:${p.total_progress}%`);
            });
        }
        console.log('');
