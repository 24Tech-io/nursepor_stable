...
async function main() {
    try {
        ...
        // 1. Get User Info
        const users = await sql`SELECT id, name, email, role, is_active FROM users WHERE id = ${studentId}`;
        if (users.length === 0) {
        ...
        if (enrollments.length > 0) {
            ...
            enrollments.forEach(e => {
                console.log(`   - Enrollment #${e.id}: [${e.status.toUpperCase()}] for "${e.course_title}" (Course ID: ${e.course_id}, Course Status: ${e.course_status})`);
            });
        } else {
        ...
        if (activeEnrollments.length > 0) {
            ...
            activeEnrollments.forEach(e => {
                console.log(`   - Enrollment #${e.id}: "${e.course_title}" (Course ID: ${e.course_id}, Course Status: ${e.course_status})`);
            });
        } else {
        ...
        if (requests.length > 0) {
            ...
            requests.forEach(r => {
                console.log(`   - Request #${r.id}: [${r.status.toUpperCase()}] for "${r.course_title}" (Course ID: ${r.course_id})`);
            });
            ...
            if (pendingRequests.length > 0) {
                pendingRequests.forEach(r => {
                    console.log(`   - Pending Request #${r.id}: "${r.course_title}" (Course ID: ${r.course_id})`);
                });
            }
        } else {
