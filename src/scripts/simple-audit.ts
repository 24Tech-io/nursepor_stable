...
(async () => {
  ...
  rogue.forEach((r) =>
    console.log(
      `  - ${r.name} → ${r.title} (student_id: ${r.student_id}, course_id: ${r.course_id})`
    )
  );
  ...
  orphan.forEach((o) =>
    console.log(`  - ${o.name} → ${o.title} (user_id: ${o.user_id}, course_id: ${o.course_id})`)
  );
  ...
  stale.forEach((s) =>
    console.log(
      `  - ${s.name} → ${s.title} (status: ${s.status}, has_enrollment: ${s.has_enrollment})`
    )
  );
