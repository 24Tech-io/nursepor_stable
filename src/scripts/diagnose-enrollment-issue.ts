...
async function main() {
    ...
    if (progressRecords.length === 0) {
        ...
        progressRecords.forEach((rec, i) => {
            console.log(`   ${i + 1}. Course ID: ${rec.courseId}, Title: ${rec.courseTitle || 'N/A'}, Progress: ${rec.totalProgress}%`);
        });
    }
    ...
    if (enrollmentRecords.length === 0) {
        ...
        enrollmentRecords.forEach((rec, i) => {
            console.log(`   ${i + 1}. Course ID: ${rec.courseId}, Title: ${rec.courseTitle || 'N/A'}, Status: ${rec.status}, Progress: ${rec.progress}%`);
        });
    }
    ...
    if (requestRecords.length === 0) {
        ...
        requestRecords.forEach((rec, i) => {
            console.log(`   ${i + 1}. Course ID: ${rec.courseId}, Title: ${rec.courseTitle || 'N/A'}, Status: ${rec.status}, Requested: ${rec.requestedAt}`);
        });
    }
    ...
    const missingInEnrollments = Array.from(progressCourseIds).filter(id => !enrollmentCourseIds.has(id));
    ...
    const missingInProgress = Array.from(enrollmentCourseIds).filter(id => !progressCourseIds.has(id));
    ...
    if (missingInEnrollments.length > 0) {
        ...
        console.log(`      Solution: Run sync-missing-enrollments.ts or manually create missing records`);
