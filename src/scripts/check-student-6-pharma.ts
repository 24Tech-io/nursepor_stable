...
(async () => {
  ...
  `;
    console.log(`Enrollment: ${enrollment.length > 0 ? 'EXISTS (id: ' + enrollment[0].id + ', status: ' + enrollment[0].status + ')' : 'NOT FOUND'}`);
  ...
  `;
    console.log(`studentProgress: ${progress.length > 0 ? 'EXISTS (id: ' + progress[0].id + ')' : 'NOT FOUND'}`);
