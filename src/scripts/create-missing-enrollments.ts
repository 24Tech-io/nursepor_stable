...
async function main() {
    ...
    `;
        ...
        `;
                ...
                if (result.length > 0) {
                    created++;
                    console.log(`✅ Created enrollment #${result[0].id} for student ${missing.student_name}, course "${missing.title}"`);
                }
            } catch (error: any) {
                console.error(`❌ Failed to create enrollment for student ${missing.student_id}, course ${missing.course_id}:`, error.message);
            }
        }
