...
export async function POST(request: NextRequest) {
  try {
    ...
    let course = await db.select().from(courses).where(eq(courses.title, 'Q-Bank')).limit(1);
    ...
    const allStudents = await db.select().from(users).where(eq(users.role, 'student')).limit(10);
    ...
    const questionTemplates = [
      ...
      {
        question:
          'The nurse is assessing a client with chronic obstructive pulmonary disease (COPD). Which finding is most indicative of respiratory distress?',
        options: [
          'Barrel chest',
          'Clubbing of fingers',
        ...
        explanation:
          'Use of accessory muscles indicates the client is working harder to breathe, which is a sign of respiratory distress.',
        difficulty: 'medium',
      },
      {
        question:
        ...
        options: [
          'Verify the blood type with another nurse',
        ...
        explanation:
          'Blood type verification with another nurse is a critical safety measure that must be done before any blood product administration.',
        difficulty: 'medium',
      },
    ];
    ...
    for (const student of allStudents) {
      ...
      const enrollment = await db
        ...
        .where(
          and(eq(studentProgress.studentId, student.id), eq(studentProgress.courseId, courseId))
        )
        .limit(1);
