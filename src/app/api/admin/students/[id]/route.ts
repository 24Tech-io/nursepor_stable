...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!decoded || decoded.role !== 'admin') {
      logger.error('❌ Invalid token or not admin');
    ...
    if (isNaN(studentId)) {
      logger.error('❌ Invalid student ID:', params.id);
    ...
    if (!userCheck) {
      logger.error('User not found with ID:', studentId);
      return NextResponse.json({ message: 'Student not found' }, { status: 404 });
    }

    if (userCheck.role !== 'student') {
      logger.error('User is not a student. Role:', userCheck.role, 'ID:', studentId);
    ...
    if (!student) {
      logger.error('Student not found after role check. ID:', studentId);
      return NextResponse.json({ message: 'Student not found' }, { status: 404 });
    }

    logger.info('✅ Student found:', student.name, student.email);
    ...
    try {
      ...
      logger.info('📋 Pending request course IDs:', Array.from(pendingRequestCourseIds));
    } catch (pendingError: any) {
      logger.error('⚠️ Error fetching pending requests (continuing without filter):', pendingError);
    ...
    try {
      ...
      logger.info(`✅ Found ${enrolledCount} valid enrollments (merged from both tables, excluded ${(enrollmentsData.length + studentProgressData.length) - enrolledCount} with pending requests)`);
    ...
    try {
      ...
      logger.error('Error fetching pending requests:', requestError);
      pendingRequests = [];
    }
