  ...
  try {
    ...
    try {
      ...
      if (tableError?.message && (tableError.message.includes('does not exist') || tableError.message.includes('relation'))) {
        logger.warn('⚠️ activity_logs table does not exist yet. Run migration to create it.');
        return NextResponse.json({ logs: [] });
      }
      // Log the actual error for debugging
      logger.error('Activity logs query error:', tableError);
    ...
    const formattedLogs = logs.map((log: any) => {
      ...
      try {
        ...
        logger.warn('Failed to parse activity log details:', e);
        details = null;
      }
