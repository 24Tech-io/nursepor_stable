...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `🔍 [Progress Details] Student ${decoded.id}: Found ${pendingRequestCourseIds.length} pending requests and ${approvedRequestCourseIds.length} approved requests`
    );
      ...
      for (const courseId of approvedRequestCourseIds) {
        try {
          ...
          if (existing.length === 0) {
            console.log(
              `🔄 [Progress Details] Syncing enrollment for approved request: student ${decoded.id}, course ${courseId}`
            );
            await syncEnrollmentAfterApproval(decoded.id, courseId);
          }
        } catch (syncError: any) {
          console.error(
            `❌ [Progress Details] Error syncing enrollment for course ${courseId}:`,
            syncError
          );
        }
      }
    ...
    try {
      ...
      console.log(
        `📊 [Progress Details] Found ${progressRecords.length} progress records from studentProgress table`
      );
      ...
      progressRecords = progressRecords.filter((p: any) => {
        ...
        if (!isValid) {
          console.log(
            `⚠️ [Progress Details] Filtering out course ${p.course.id} (${p.course.title}) - status: ${p.course.status}`
          );
        }
        return isValid;
      });
      ...
      if (!enrolledCoursesMap.has(e.courseId)) {
        ...
        try {
          ...
          if (existingProgress.length === 0) {
            ...
            console.log(
              `✅ Created progress record for course ${e.courseId} for student ${decoded.id}`
            );
          }
        } catch (error: any) {
    ...
    const filteredProgress = allProgress.filter((p: any) => {
      ...
      if (hasPendingRequest) {
        console.log(
          `⚠️ [Progress Details] Filtering out course ${p.courseId} (${p.course.title}) - has pending request`
        );
      }
    ...
    console.log(
      `✅ [Progress Details] Total enrolled courses: ${allProgress.length}, After filtering pending requests: ${filteredProgress.length}`
    );
    ...
    console.log(
      `✅ [Progress Details] Returning ${progressList.length} progress items for student ${decoded.id}`
    );
    if (progressList.length > 0) {
      console.log(
        `📋 [Progress Details] Progress items:`,
