...
export default function EditQuestionPage() {
  ...
  const fetchQuestion = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const question = data.questions.find((q: any) => q.id === parseInt(questionId));

        if (question) {
          const options = safeJsonParse(question.options, []);
          const correctAnswer = safeJsonParse(question.correctAnswer, 'a');

          setFormData({
            question: question.stem || question.fullQuestion || '',
            explanation: question.explanation || '',
            questionType: question.type || question.questionType || 'multiple_choice',
            testType: question.testType || 'mixed',
            difficulty: question.difficulty || 'medium',
            subject: question.subject || '',
            lesson: question.lesson || '',
            clientNeedArea: question.clientNeedArea || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!formData.question || formData.question.replace(/<[^>]*>/g, '').trim().length < 10) {
