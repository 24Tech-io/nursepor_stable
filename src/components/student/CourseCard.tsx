      ...
      if (response.ok) {
        ...
        if (course) {
          ...
          if (course.isEnrolled) {
            ...
            // Check enrollment status in background without reload
            setTimeout(async () => {
              try {
                const checkResponse = await fetch('/api/student/courses', {
                  credentials: 'include',
                });
                if (checkResponse.ok) {
                  const checkData = await checkResponse.json();
                  const updatedCourse = checkData.courses?.find((c: any) => c.id === String(courseId));
                  if (updatedCourse?.isEnrolled) {
  ...
  return (
    <div>
      ...
      <button
        ...
        className={`w-full font-semibold py-3 rounded-xl transition-all duration-300 shadow-md hover:shadow-glow-red transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 ${requestSubmitted
            ? 'bg-gradient-to-r from-nurse-silver-600 to-nurse-silver-700 text-white cursor-not-allowed'
...
export default function CourseCard({
  ...
  // Prefetch course detail page on hover
  const handleMouseEnter = () => {
    if (!finalIsLocked) {
      router.prefetch(`/student/courses/${course.id}`);
    }
  };

  return (
    <div
      className="group bg-white/5 backdrop-blur-xl rounded-2xl shadow-dark-lg hover:shadow-glow-red transition-all duration-300 overflow-hidden border border-white/10 hover:border-nurse-red-500/30"
      onMouseEnter={handleMouseEnter}
    >
      {/* Thumbnail with Overlay */}
      <div className="relative h-52 overflow-hidden bg-gradient-to-br from-nurse-red-900/30 to-slate-900">
        {course.thumbnail ? (
          <img
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg className="w-16 h-16 text-nurse-red-500/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {finalIsLocked && (
          <div className="absolute inset-0 bg-black/70 flex items-center justify-center backdrop-blur-sm">
            <div className="text-center">
              <div className="w-16 h-16 bg-nurse-red-500/20 rounded-full flex items-center justify-center mx-auto mb-2 border border-nurse-red-500/30">
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-semibold text-nurse-red-400 bg-nurse-red-500/20 px-3 py-1 rounded-full border border-nurse-red-500/30">
        ...
        <h3 className="text-xl font-bold text-white mb-2 line-clamp-2 group-hover:text-nurse-red-400 transition-colors">
          {course.title}
        </h3>

        <p className="text-sm text-nurse-silver-400 mb-4 line-clamp-2 leading-relaxed">
          {course.description}
        </p>
        ...
        {finalIsLocked ? (
          ...
          <Link href={`/student/courses/${course.id}`}>
            <button className="w-full bg-gradient-to-r from-nurse-red-600 to-red-600 text-white font-semibold py-3 rounded-xl hover:from-nurse-red-700 hover:to-red-700 transition-all duration-300 shadow-md hover:shadow-glow-red transform hover:-translate-y-0.5 flex items-center justify-center space-x-2">
