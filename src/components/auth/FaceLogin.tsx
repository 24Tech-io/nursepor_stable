  ...
  const requestCameraPermission = async () => {
    ...
    try {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        errorMsg =
          'Camera permission denied. Please click the camera icon in your browser\'s address bar and select "Allow", then try again.';
        setNeedsPermission(true);
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-purple-900 to-blue-900 bg-opacity-90">
              <div className="text-center text-white p-8">
                ...
                <p className="text-sm text-gray-300 mb-4">
                  Your privacy is protected - no images are stored
                </p>
              </div>
            </div>
          )}
