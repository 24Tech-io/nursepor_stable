                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="bg-[#161922] border border-slate-800/60 rounded-2xl p-6">
                        ...
                        <div className="mt-3 flex items-center gap-2">
                            <span className="text-green-400 text-xs font-semibold">{studentStats?.active || 0} Active</span>
                            <span className="text-slate-600">•</span>
                            <span className="text-red-400 text-xs font-semibold">{studentStats?.inactive || 0} Inactive</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#161922] border border-slate-800/60 rounded-2xl p-6">
                        ...
                        <h3 className="text-3xl font-bold text-white mb-1">{studentStats?.recentActiveCount || 0}</h3>
                    ...
                    <div className="bg-[#161922] border border-slate-800/60 rounded-2xl p-6">
                        ...
                        <h3 className="text-3xl font-bold text-white mb-1">{engagementStats?.totalWatchTimeHours || '0'}</h3>
                    ...
                    <div className="bg-[#161922] border border-slate-800/60 rounded-2xl p-6">
                        ...
                        <h3 className="text-3xl font-bold text-white mb-1">{engagementStats?.videoStats.completionRate || '0'}%</h3>
                        <p className="text-slate-400 text-sm">Video Completion</p>
                        <p className="text-slate-600 text-xs mt-1">
                            {engagementStats?.videoStats.completed || 0} / {engagementStats?.videoStats.total || 0} videos
                        </p>
                    </div>
                </div>
                    ...
                    <div className="bg-[#161922] border border-slate-800/60 rounded-2xl p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-white font-bold">{engagementStats?.avgStudentProgress || '0'}%</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-slate-400">Videos Watched</span>
                                <span className="text-white font-bold">{engagementStats?.videoStats.completed || 0}</span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-green-400 font-semibold">
                                        {studentStats ? ((studentStats.active / studentStats.total) * 100).toFixed(1) : 0}%
                                    </span>
                                </div>
                                <div className="w-full bg-slate-800 rounded-full h-2">
                                    <div
                                        className="bg-green-500 h-2 rounded-full"
                                        style={{ width: `${studentStats ? (studentStats.active / studentStats.total) * 100 : 0}%` }}
                                    ></div>
                                </div>
                            </div>
                            <div>
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-red-400 font-semibold">
                                        {studentStats ? ((studentStats.inactive / studentStats.total) * 100).toFixed(1) : 0}%
                                    </span>
                                </div>
                                <div className="w-full bg-slate-800 rounded-full h-2">
                                    <div
                                        className="bg-red-500 h-2 rounded-full"
                                        style={{ width: `${studentStats ? (studentStats.inactive / studentStats.total) * 100 : 0}%` }}
                                    ></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-slate-800">
                                    <th className="text-left py-4 px-4 text-slate-400 font-semibold text-sm">Course</th>
                                    <th className="text-left py-4 px-4 text-slate-400 font-semibold text-sm">Enrollments</th>
                                    <th className="text-left py-4 px-4 text-slate-400 font-semibold text-sm">Avg Progress</th>
                                    <th className="text-right py-4 px-4 text-slate-400 font-semibold text-sm">Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                {enrollmentData.map((course) => (
                                    <tr key={course.courseId} className="border-b border-slate-800/40 hover:bg-slate-800/20 transition">
                                        ...
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-white font-semibold text-sm">{Number(course.avgProgress).toFixed(1)}%</span>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4 text-right">
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${course.enrollmentCount > 10
