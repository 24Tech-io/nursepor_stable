...
async function testStudentAPI() {
    ...
    enrollmentsData.forEach((e, i) => {
        console.log(`  ${i + 1}. Course ID: ${e.courseId}, Title: ${e.course.title}, Status: ${e.status}, Course Status: ${e.course.status}`);
    });

    // Filter by course status
    const publishedEnrollments = enrollmentsData.filter((e) =>
    ...
    publishedEnrollments.forEach((e, i) => {
        console.log(`  ${i + 1}. Course ID: ${e.courseId}, Title: ${e.course.title}, Course Status: ${e.course.status}`);
    });
