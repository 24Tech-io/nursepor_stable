...
export default function ExtendedDragDrop({ question, onChange }: { question: any; onChange: (q: any) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Drop Zones / Categories</label>
        {categories.map((cat: string, idx: number) => (
          <div key={idx} className="mb-3">
            <input
              ...
              className="w-full mb-2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
            />
            <div className="bg-gray-50 rounded-lg p-3 min-h-[60px] border-2 border-dashed border-gray-300">
              <p className="text-xs text-gray-500 mb-2">Items that belong here:</p>
